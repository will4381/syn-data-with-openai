...
def process_inferences_to_csv(inference_files, passages, output_file):
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['passage', 'inference1', 'confidence1', 'inference2', 'confidence2', 
                      'inference3', 'confidence3', 'inference4', 'confidence4', 'inference5', 'confidence5']
                    ...
                    try:
                        ...
                        if 'response' in response and response['response']['status_code'] == 200:
                            ...
                            for i, inference in enumerate(inferences, 1):
                                row[f'inference{i}'] = inference['text']
                                row[f'confidence{i}'] = inference['confidence']
